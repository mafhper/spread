...
export function useColorExtractor() {
    ...
    const extractColorsFromImage = useCallback(async (imageUrl: string): Promise<ExtractedColors | null> => {
        setIsExtracting(true);
        setError(null);
        console.log('[useColorExtractor] Starting extraction for:', imageUrl.substring(0, 50) + '...');
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                try {
                    ...
                    const secondary = palette.length > 1 
                        ? rgbToHex(palette[1][0], palette[1][1], palette[1][2])
                        : darkenColor(primary, 30);
                    
                    const hexPalette = palette.map((c: number[]) => rgbToHex(c[0], c[1], c[2]));
                    
                    console.log('[useColorExtractor] Extracted colors:', { primary, secondary, palette: hexPalette });
            ...
            img.onerror = () => {
                console.error('[useColorExtractor] Image failed to load:', imageUrl.substring(0, 50));
