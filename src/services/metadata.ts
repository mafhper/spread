...
async function fetchYouTubeData(url: string): Promise<{ title: string; author: string; thumbnail: string } | null> {
    try {
...
function parseMusircTitle(title: string, author: string): { cleanTitle: string; artist: string } {
...
function detectTemplate(url: string, author: string): 'default' | 'music' | 'news' {
    const musicDomains = ['music.youtube', 'spotify', 'soundcloud', 'bandcamp', 'deezer', 'tidal', 'apple.com/music'];
    const newsDomains = ['bbc', 'cnn', 'reuters', 'nytimes', 'washingtonpost', 'theguardian', 'g1.globo', 'uol.com.br/noticias'];
    
    const urlLower = url.toLowerCase();
    
    if (musicDomains.some(d => urlLower.includes(d))) return 'music';
    if (urlLower.includes('youtube') && (urlLower.includes('watch') || urlLower.includes('youtu.be'))) return 'music';
...
export async function fetchMetadata(url: string): Promise<LinkMetadata | null> {
    try {
        ...
        const isYouTube = targetUrl.includes('youtube') || targetUrl.includes('youtu.be');
        ...
        if (json.status === 'success') {
            ...
            favicon = data.logo?.url || `https://www.google.com/s2/favicons?domain=${domain}&sz=64`;
