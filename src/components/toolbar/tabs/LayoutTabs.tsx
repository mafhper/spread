...
export const LayoutTabs: React.FC = () => {
    ...
    const updateLayout = (field: string, val: any) => updateNestedField('layout', field, val);
    ...
    return (
        <details className="group" open={isOpen} onToggle={(e) => setIsOpen(e.currentTarget.open)}>
            <summary className="flex items-center justify-between font-medium text-sm text-[var(--text-muted)] uppercase tracking-wider mb-3 group-hover:text-[var(--text-main)] transition-colors cursor-pointer list-none">
                <span className="flex items-center gap-2"><Layout size={16}/> Layout do Card</span>
             ...
             <div className="space-y-6 pl-1 pt-2">
                 ...
                 <div className="space-y-4 pt-2">
                    <h4 className="text-[10px] font-bold text-white/30 uppercase tracking-[0.1em]">1. Proporções do Card</h4>
                    
                    {/* Card Aspect Ratio */}
                    <div>
                        <label className="block text-xs font-medium mb-1.5 text-[var(--text-muted)]">Formato do Card</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="text-xs font-medium text-[var(--text-muted)]">Tamanho do Card (Escala)</label>
                            <span className="text-xs text-[var(--accent-primary)] font-mono">{Math.round(layout.cardScale * 100)}%</span>
                        </div>
                        <input 
                 ...
                 <div className="space-y-4 pt-2 border-t border-white/5">
                    <h4 className="text-[10px] font-bold text-white/30 uppercase tracking-[0.1em]">2. Estilo e Transparência</h4>
                 ...
                 {/* 3. Shadow & Depth */}
                 <div className="space-y-4 pt-2 border-t border-white/5">
                    <h4 className="text-[10px] font-bold text-white/30 uppercase tracking-[0.1em]">3. Sombra e Profundidade</h4>
                 ...
                 <div className="space-y-4 pt-2 border-t border-white/5">
                    <h4 className="text-[10px] font-bold text-white/30 uppercase tracking-[0.1em]">4. Foto do Link</h4>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="text-xs font-medium text-[var(--text-muted)]">Zoom da Foto</label>
                            <span className="text-xs text-[var(--accent-primary)] font-mono">{Math.round(layout.imageScale * 100)}%</span>
                        </div>
                        <input 
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="text-[10px] font-medium text-[var(--text-muted)]">Ajuste X</label>
                            <span className="text-[10px] text-[var(--accent-primary)] font-mono">{layout.imageOffsetX}px</span>
                        </div>
                        <input 
                        ...
                        <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="text-[10px] font-medium text-[var(--text-muted)]">Ajuste Y</label>
                            <span className="text-[10px] text-[var(--accent-primary)] font-mono">{layout.imageOffsetY}px</span>
                        </div>
                        <input 
                            type="range" min="-400" max="400" step="1"
