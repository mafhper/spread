...
vi.mock('colorthief', () => {
  return {
    default: class {
      getColor = vi.fn().mockReturnValue([255, 0, 0])
      getPalette = vi.fn().mockReturnValue([
        [255, 0, 0],
        [0, 255, 0],
      ])
    },
...
describe('useColorExtractor hook', () => {
  ...
  it('should darken color for secondary when palette lacks color', async () => {
    // Mock ColorThief to return only one color
    vi.unmock('colorthief')
    vi.mock('colorthief', () => {
      return {
        default: class {
          getColor = vi.fn().mockReturnValue([100, 150, 200])
          getPalette = vi.fn().mockReturnValue([[100, 150, 200]]) // Only one color
        },
      }
    })
